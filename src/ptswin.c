...
int track_set = SET_SELECT_NEAREST;
int track_point = -1;
...
static void do_find_proc(Widget w, XtPointer client_data, XtPointer call_data)
{
	if( track_set == SET_SELECT_ERROR ){
		errwin( "No points to find" );
		return;
	}
	set_action(0);
...
static void do_gotoxy_proc(Widget w, XtPointer client_data, XtPointer call_data)
{
    ...
    if(xv_evalexpr(goto_pointx_item, &wx) != GRACE_EXIT_SUCCESS ||
       xv_evalexpr(goto_pointy_item, &wy) != GRACE_EXIT_SUCCESS  ) {
	return;
    }
...
*/
	...
	XtManageChild(rc);
	XtAddCallback(locate_set_item.list, XmNsingleSelectionCallback, 
					(XtCallbackProc) update_track_set, (XtPointer) 0);
	XmListSelectPos( locate_set_item.list, 1, True );
...
static void do_digit_proc(Widget w, XtPointer client_data, XtPointer call_data)
{
    ...
    x = getx(cg, setno);
    y = gety(cg, setno);
	xv_evalexpr( digit_p1x_item, &p1x );
	xv_evalexpr( digit_p1x_item, &p1y );
	xv_evalexpr( digit_p1x_item, &p2x );
	xv_evalexpr( digit_p1x_item, &p2y );
	xv_evalexpr( digit_p1x_item, &p3x );
	xv_evalexpr( digit_p1x_item, &p3y );
