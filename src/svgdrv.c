...
int svginitgraphics(void)
{
    ...
    fprintf(prstream, "<?xml version=\"1.0\" encoding=\"ISO-8859-1\" standalone=\"no\"?>\n");
    fprintf(prstream,
            "<!DOCTYPE svg PUBLIC \"-//W3C//DTD SVG December 1999//EN\"\n");
    fprintf(prstream,
            "  \"http://www.w3.org/Graphics/SVG/SVG-19991203.dtd\">\n");
    fprintf(prstream, "<!-- generated by %s -->\n", bi_version_string());
    fprintf(prstream, "<svg width=\"%f\" height=\"%f\">\n",
            (double) page_width_pp, (double) page_height_pp);
...
void svg_drawpolyline(VPoint *vps, int n, int mode)
{
    ...
    fprintf(prstream, "   <path d=\"M%f,%f",
            convertX(vps[0].x), convertY(vps[0].y));
    for (i = 1; i < n; i++) {
        if (i%10 == 0) {
            fprintf(prstream, "\n            ");
        }
        fprintf(prstream, "L%f,%f", convertX(vps[i].x), convertY(vps[i].y));
    }
